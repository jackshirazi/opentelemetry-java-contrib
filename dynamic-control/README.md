...
policy type, detailed above, and because ituses a sampling rate applied globally, the id for it
...
The pipeline initialization could still map the source key to one implementation:
...
sources:
  - kind: opamp
    ...
    location: vendor
    mappings:
      - sourceKey: trace_sampling_rules
        policyType: trace-sampling-per-span
```

The source payload could then contain multiple policies with the same type but different ids
(note `matching` is defined in the spec, but not yet implemented here):

```json
{
  "trace-sampling-per-span": {
