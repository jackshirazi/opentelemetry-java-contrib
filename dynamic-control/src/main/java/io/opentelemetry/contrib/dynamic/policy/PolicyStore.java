...
public final class PolicyStore {
   ...
   * are dropped and only the first occurrence of each policy is kept
  ...
  private static List<TelemetryPolicy> deletedPoliciesFrom(
    ...
    for (TelemetryPolicy previousPolicy : previousPolicies) {
      if (!activePolicyKeys.contains(policyKey(previousPolicy))) {
        deletedPolicies.add(
            new DeletedTelemetryPolicy(previousPolicy.getIdentity(), previousPolicy.getType()));
      }
    }
